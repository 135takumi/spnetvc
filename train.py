...
def calc_ld_loss(model, x_t, label,  optimizer):
    # ldの学習
...
def train(model, vae_optimizer, cts_ld_optimizer, vae_train_loader, cts_ld_train_loader,
          writer, epoch, debug):
    model.train()
    ...
    for (vae_x_t1, vae_x_t2, vae_label), (cts_ld_x_t, _, cts_ld_label), \
            in zip(vae_train_loader, cts_ld_train_loader):

        vae_x_t1, vae_label = vae_x_t1.to(hp.device), vae_label.to(hp.device)
        ...
        # vaeのloss
        loss = hp.rec_lambda * rec_loss + + hp.cts_kl_lambda * cts_kl_loss \
               + hp.atr_kl_lambda * atr_kl_loss \
               - cts_lambda * cts_ld_loss
...
def valid(model, clf, valid_loader, writer, epoch, debug):
    ...
    with torch.no_grad():
        for x_t1, _, label in valid_loader:
            x_t1, label = x_t1.to(hp.device), label.to(hp.device)
...
def main():
    ...
    for epoch in tqdm(range(1, hp.spnetvc_epochs + 1)):
        train(model, vae_optimizer, cts_ld_optimizer, vae_train_loader, cts_ld_train_loader,
              writer, epoch, args.debug)
